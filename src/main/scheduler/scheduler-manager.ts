...
export class SchedulerManager {
  ...
  private checkSchedules(): void {
    ...
    for (const [id, task] of this.schedules) {
      ...
      if (task.scheduleType === "once" && task.datetime) {
        const taskTime = new Date(task.datetime).getTime();
        const nowTime = now.getTime();
        if (nowTime >= taskTime && (!task.lastRun || new Date(task.lastRun).getTime() < taskTime)) {
  ...
  addSchedule(options: Partial<SchedulerTask>): SchedulerTask {
    ...
    const task: SchedulerTask = {
      ...
      name: options.name || "",
      url: options.url || "",
      scheduleType: options.scheduleType || "cron",
