import { app, BrowserWindow, session, Menu, Tray, nativeImage, shell, protocol } from "electron";
...
app.whenReady().then(async () => {
  try {
    ...
    // Register media protocol for local file playback
    protocol.registerFileProtocol("media", (request, callback) => {
      const url = request.url.replace("media://", "");
      try {
        return callback({ path: decodeURIComponent(url) });
      } catch (error) {
        log.error("Failed to decode media URL:", error);
    ...
    scheduler.start();
    rssManager = new RssManager(settings, downloadEngine);
    rssManager.start();
    ...
    const watchFolder = new WatchFolderManager(settings, mainWindow);
    watchFolder.start();
