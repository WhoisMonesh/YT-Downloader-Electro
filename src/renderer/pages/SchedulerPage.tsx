...
export default function SchedulerPage() {
  ...
  const [form, setForm] = useState({ name: "", url: "", cronExpression: "0 * * * *", outputFormat: "mp4" as SchedulerTask["outputFormat"], quality: "best" as SchedulerTask["quality"], action: "none" as SchedulerTask["action"] });

  useEffect(() => { window.electronAPI.getScheduledTasks().then(setTasks); }, []);

  const handleAdd = useCallback(async () => {
    if (!form.name || !form.url) return;
    const task = await window.electronAPI.addScheduledTask(form);
    setTasks((p) => [...p, task]);
    setForm({ name: "", url: "", cronExpression: "0 * * * *", outputFormat: "mp4", quality: "best", action: "none" });
    setShowForm(false);
  }, [form]);
      ...
      {showForm && (
        <div className="glass-panel p-6 space-y-4 animate-fade-in">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs text-accent-500 mb-1">Schedule (Cron)</label>
              <input type="text" value={form.cronExpression} onChange={(e) => setForm({ ...form, cronExpression: e.target.value })}
                className="w-full h-10 px-4 rounded-lg bg-accent-900/50 border border-accent-800 text-accent-50 font-mono text-sm focus:outline-none focus:ring-2 focus:ring-blue/50" />
            </div>
            <div>
          ...
          <div key={t.id} className="glass-panel p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-accent-600 mt-1">Cron: {t.cronExpression} - {t.outputFormat.toUpperCase()}</p>
              </div>
